...
import { __t } from './download-torrent.bg'
  ...
  chrome.scripting.executeScript({
      ...
      func: async (bookToProcess: IBookInfo, BASE_API_URL: string/*, tabId: number*/) => {
        ...
        try {
          ...
          // todo remove
          console.log(' *---> PAGE. info posted', res.ok ? await res.json() : 'NOT OK <<')
    ...
    /*(el) => {
      /!* NB! resolve(el[0].result)

      setTimeout(() => {
        chrome.tabs.remove(tab.id!)
      }, calcRandomPause(60_000, 3 * 60_000, 10 * 60_000))*!/
      // // todo remove
      // console.log(' *---> cb().EL', el)
      // const _filename = el[0].result

      // oFileName['a'] = _filename
    },*/
  )
  // })
}
